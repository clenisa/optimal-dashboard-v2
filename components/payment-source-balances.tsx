...
export function PaymentSourceBalances() {
  ...
  // Show test data option if no real data with credit limits
  const hasRealCreditData = validSources.some(source => 
    (source as PaymentSource).max_balance && (source as PaymentSource).type === 'credit'
  )

  if (!hasRealCreditData) {
    return (
      <Card className="w-full h-full">
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          {!useTestData && (
            <div className="text-sm text-gray-500">
              No credit account data with limits available. Enable test data to see the visualization.
            </div>
          )}
